#!/usr/bin/env bash
# Run any coreutils utility under KLEE with EVP/VASE enabled.
# Usage:
#   ./run_evp.sh <utility> [-- extra app args...]
#
# Env overrides (optional): same as run_vanilla.sh, plus
#   VASE_MAP=$ROOT/evp_artifacts/<prog>/limitedValuedMap.json
#
set -euo pipefail
...
[[ -f "$VASE_MAP" ]] || die "Missing VASE map: $VASE_MAP (run Step-1 collection first)"
...
OUT_DIR_EVP="${KLEE_OUT_BASE}/${PROG}/evp-${RUN_TAG}"
SANDBOX="$(mktemp -d /tmp/${PROG}-evp-XXXXXX)"
trap 'rm -rf "$SANDBOX"' EXIT

note "Output -> $OUT_DIR_EVP"
mkdir -p "$OUT_DIR_EVP"
[[ -f "$TEST_ENV" ]] || note "Warning: env file missing ($TEST_ENV); continuing without it"

# KLEE invocation (with EVP)
"$KLEE_BIN" \
  --use-vase --vase-map="$VASE_MAP" \
